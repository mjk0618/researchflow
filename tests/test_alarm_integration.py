...
class AlarmIntegrationTests(unittest.TestCase):
    ...
    def test_log_wait_keeps_alarm_foreground_and_writes_log(self):
        repo_root = Path(__file__).resolve().parents[1]
        sample_script = repo_root / "tests" / "fixtures" / "sample_experiment.py"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            log_files = list(Path(tmpdir).glob("*_sample_experiment.log"))

        self.assertEqual(completed.returncode, 0, completed.stderr)
        self.assertIn("Logging stdout/stderr to:", completed.stdout)
        self.assertIn("Skipping Slack notification", completed.stdout)
        self.assertEqual(len(log_files), 1)

    def test_default_log_path_uses_script_logs_dir_and_kst_timestamp(self):
        script_path = "/tmp/project/train.py"
        with mock.patch("researchflow.alarm.handler._get_kst_log_timestamp", return_value="20260502_123456_123456_KST"):
            log_path = _resolve_log_file_path(script_path)

        self.assertEqual(log_path, "/tmp/project/logs/20260502_123456_123456_KST_train.log")
    ...
    def test_custom_log_path_uses_configured_dir(self):
        ...
        self.assertEqual(log_path, "/tmp/researchflow_logs/20260502_123456_123456_KST_train.log")

    def test_log_path_timestamp_includes_microseconds_to_avoid_fast_loop_collision(self):
        script_path = "/tmp/project/train.py"
        ...
        self.assertRegex(
            ...
            r"^\d{8}_\d{6}_\d{6}_KST_train\.log$",
        )
